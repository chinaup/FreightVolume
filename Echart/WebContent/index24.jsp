<%@ page language="java" import="FreightVolume.connDB,java.util.*" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%
ArrayList<String[]> list = connDB.everyProvinceDeliverVolume();
%>    
    ...
    <div class="content">
        <div class="nav">
            <ul>
                ...
                <li class="current"><a href="#">区域发货量热力图</a></li>
            </ul>
        </div>
        <div class="container">
            <div class="title">区域发货量热力图</div>
            <div class="show">
	    ...
	    series : [
	        {
	            ...
	            data:[
	            	{name: '广东', value: <%=list.get(0)[1]%>},
	            	{name: '山东', value: <%=list.get(1)[1]%>},
	            	{name: '河南', value: <%=list.get(2)[1]%>},
	            	{name: '湖北', value: <%=list.get(3)[1]%>},
	            	{name: '云南', value: <%=list.get(4)[1]%>},
	            	{name: '内蒙古', value: <%=list.get(5)[1]%>},
	            	{name: '重庆', value: <%=list.get(6)[1]%>},
	            	{name: '江西', value: <%=list.get(7)[1]%>},
	            	{name: '河北', value: <%=list.get(8)[1]%>},
	            	{name: '福建', value: <%=list.get(9)[1]%>},
	            	{name: '江苏', value: <%=list.get(10)[1]%>},
	            	{name: '上海', value: <%=list.get(11)[1]%>},
	            	{name: '浙江', value: <%=list.get(12)[1]%>},
	            	{name: '山西', value: <%=list.get(13)[1]%>},
	            	{name: '黑龙江', value: <%=list.get(14)[1]%>},
	            	{name: '广西', value: <%=list.get(15)[1]%>},
	            	{name: '陕西', value: <%=list.get(16)[1]%>},
	            	{name: '新疆', value: <%=list.get(17)[1]%>},
	            	{name: '安徽', value: <%=list.get(18)[1]%>},
	            	{name: '天津', value: <%=list.get(19)[1]%>},
	            	{name: '甘肃', value: <%=list.get(20)[1]%>},
	            	{name: '宁夏', value: <%=list.get(21)[1]%>},
	            	{name: '四川', value: <%=list.get(22)[1]%>},
	            	{name: '北京', value: <%=list.get(24)[1]%>},
	            	{name: '辽宁', value: <%=list.get(25)[1]%>},
	            	{name: '湖南', value: <%=list.get(26)[1]%>},
	            	{name: '吉林', value: <%=list.get(27)[1]%>},
	            	{name: '青海', value: <%=list.get(28)[1]%>},
	            	{name: '海南', value: <%=list.get(29)[1]%>},
	            	{name: '贵州', value: <%=list.get(30)[1]%>},   
	            	{name: '西藏', value: <%=list.get(31)[1]%>}
	            ]
	        }
	    ]
	};
